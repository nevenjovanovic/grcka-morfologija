...
let $name := "Panegyricus 9 cum commento"
let $urn0 := "urn:cite2:cex:unizghr.ffzghrpos:isocratispanegyricus"
...
let $worktitle := "Panegyricus 47"
let $verlabel := "Ex editione George Norlin (1980-1986) in collectione OGL"
...
let $urn := "tlg0010.tlg011"
let $db := "grc-morf-pos"
for $doc in collection($db)//*:treebank
where matches(db:path($doc),$urn)
let $cts1 := "urn:cts:greekLit:" || replace(db:path($doc),":47.xml",".token:")
let $cts1a := "urn:cts:greekLit:" || replace(db:path($doc),":47.xml",".postag:")
let $cts1b := "urn:cts:greekLit:" || replace(db:path($doc),":47.xml",".lemma:")
let $cts2 := "47"
let $cts1t := $cts1 || $cts2
